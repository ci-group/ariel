...
Quick start
-----------
    nohup uv run examples/spear/10_drone_evo_overnight.py \\
        --device cuda:0 --seed 42 \\
        > __data__/overnight.log 2>&1 &

Visualise results in the morning:
    uv run examples/e_drones_ec/6_visualize_evo_results.py \\
        --run-dir __data__/drone_evo_overnight/<RUN_ID>

Based on 9_drone_evo_configurable.py — see that script and README_drone_evo.md
for a full explanation of every setting.
...
parser = argparse.ArgumentParser(
    description="Overnight drone morphology evolution (pop=20, 2M PPO steps, no symmetry)"
)
parser.add_argument("--device",  default="cpu",
                    help="Torch device: 'cpu' or 'cuda:0'  (default: cpu)")
parser.add_argument("--seed",    type=int, default=42)
parser.add_argument("--out-dir", default=f"__data__/drone_evo_overnight/{curr_time}")
