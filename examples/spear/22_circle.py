"""Train a PPO circle racing controller for a drone loaded from a DroneBlueprint JSON.

Run:
    uv run examples/spear/22_circle.py --blueprint path/to/blueprint.json
    uv run examples/spear/22_circle.py --blueprint path/to/blueprint.json --device cuda:0

Mirrors the structure of 18_hover.py but trains on the `circle` gate track
