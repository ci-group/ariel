"""Visualise a quintic gate track in MuJoCo — no drone, gates only.

Spawns a bare MuJoCo world with gate markers (semi-transparent orange slabs +
red centre spheres) and a blue start-position marker.
...
Gate orientation
----------------
After a yaw rotation around Z by angle θ:
  local X = (cos θ, sin θ, 0)  ← travel direction
  local Y = (−sin θ, cos θ, 0) ← perpendicular to travel

For a MuJoCo box with size=[a, b, c] the face normals point along local ±X,
±Y, ±Z respectively.  The gate face (which the drone must fly through) must be
perpendicular to the travel direction, i.e. its normal must equal (cos θ, sin θ).
That means the thin dimension must be X  →  size=[depth, half, half].

Yaw source (generated mode)
---------------------------
Gate yaws come directly from the planner's compute_yaws() — the tangent to the
dense quintic path at each sampled gate index — NOT from the gate-to-gate
difference vector (which is noisy when gates are close together).
"""
...
parser = argparse.ArgumentParser(description="Visualise a quintic gate track in MuJoCo")
...
parser.add_argument("--dense-steps", type=int,   default=1000,
                    help="Points in the dense quintic path before subsampling (default 1000)")
...
if _gpos_path is not None and _gyaw_path is not None:
    ...
    xy_dense  = paths[0] * args.path_scale   # (dense_steps, 2)
    yaw_dense = yaws[0]                       # (dense_steps,) — tangent angles from planner
...
for i, (pos, yaw) in enumerate(zip(gate_pos, gate_yaws)):
    ...
    # Gate plane: thin dimension is LOCAL X  (= travel direction after yaw rotation).
    # After yaw rotation: local X = (cos yaw, sin yaw, 0) = travel direction.
    # Face normal ∥ local X  →  face is perpendicular to travel  →  drone flies through it. ✓
    ...
    # Green cylinder placed 0.5 m BEHIND the gate (where the drone approaches from)
    # pointing in the travel direction — shows the required approach vector.
    ax = float(-math.cos(yaw))  # behind = opposite travel direction
    ...
    arr_body.add_geom(
        ...
        # Cylinder local axis is Z; orient it along travel (local X of gate frame)
        # by adding a 90° pitch in the arrow body's own frame.
        size=[0.035, 0.22, 0.0],
        # Rotate 90° around local Y so the cylinder axis → local X = travel direction.
