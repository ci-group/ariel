...
from ariel.ec.a004 import EA, EAStep, EASettings
...
parser.add_argument('--budget', type=int, default=80, help='Number of generations')
...
class Evolution:
    ...
    def map_genotype_to_body(self, genome_data: dict | Genome) -> mujoco.MjSpec | None:
        """Decodes CPPN into a MuJoCo Body Spec."""
        genome = Genome.from_dict(genome_data) if isinstance(genome_data, dict) else genome_data
    ...
    def create_individual(self) -> Individual:
        ...
        while True:
            try:
                ...
                for _ in range(3):
                    genome.mutate(1.0, 1.0, self.id_manager.get_next_innov_id, self.id_manager.get_next_node_id)
    ...
    def reproduction(self, population: Population) -> Population:
        ...
        if not parents:
            console.log("[yellow]Warning: No ps-tagged individuals, using entire population as parents[/yellow]")
            parents = population
        ...
        while len(population) + len(new_offspring) < target_pool:
            ...
            if use_sexual:
                ...
                c_ctrl = parent.genotype['ctrl'].copy() if isinstance(parent.genotype['ctrl'], list) else parent.genotype['ctrl']
            ...
            while not valid_child and attempts < 20:
                mutant = c_morph.copy()
                mutant.mutate(0.8, 0.5, self.id_manager.get_next_innov_id, self.id_manager.get_next_node_id)
    ...
    def parent_selection(self, population: Population) -> Population:
        ...
        console.log(f"[cyan]Parent Selection: {ps_count}/{len(population)} marked for reproduction[/cyan]")
    ...
    def survivor_selection(self, population: Population) -> Population:
        ...
        # Diagnostics
        avg_fitness = np.mean([ind.fitness_ for ind in survivors if ind.fitness_ is not None and ind.fitness_ != float('inf')])
        console.log(f"[green]Survivor Selection: Avg fitness = {avg_fitness:.4f}[/green]")
        ...
        for ind in population:
            ...
            if "nodes" in g:
                for n in g["nodes"].values():
                    ...
                    if nid and nid > max_nid: max_nid = nid
            if "connections" in g:
                for c in g["connections"]:
                    inn = c.get("innovation", c.get("innov_id"))
                    if inn and inn > max_inn: max_inn = inn
        self.id_manager._node_id = max_nid
        self.id_manager._innov_id = max_inn

    def fast_physics_runner(self, model: mujoco.MjModel, data: mujoco.MjData, duration: float) -> None:
        """
    ...
    def run_simulation(self, mode: ViewerTypes, ind: Individual) -> float:
        ...
        if mode != "simple":
            console.log(f"[green]Simulating with {model.nu} joints (Target: {expected_joints})[/green]")
        ...
        match mode:
            ...
            case "video":
                recorder = VideoRecorder(output_folder=str(DATA / "videos"), file_name=f"dual_{ind.id}")
        ...
        delay_time = min(1.0, DURATION)  # Delay is always 1 second (or full duration if shorter)
        ...
        if tracker.history['xpos']:
            ...
            if traj:
                ...
                dist = np.sqrt(np.sum((effective_pos[:2] - TARGET_POSITION[:2])**2))
                
                if mode != "simple":
                    console.log(f"[blue]Traj len: {len(traj)}, start_idx: {start_idx}, movement: {np.linalg.norm(valid_movement_vector):.3f}[/blue]")
...
def main() -> None:
    console.rule("[bold purple]Starting Joint Evolution (Morph + Ctrl)[/bold purple]")
