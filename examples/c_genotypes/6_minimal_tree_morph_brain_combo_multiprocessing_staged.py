...
parser.add_argument("--morph-weight-start", type=float, default=0.05, help="Starting morphology-term weight")
parser.add_argument("--morph-weight-end", type=float, default=0.30, help="Final morphology-term weight")
parser.add_argument(
    "--reeval-top-frac",
    type=float,
    default=0.25,
    help="Top fraction of individuals reevaluated with a larger inner-learning budget",
)
parser.add_argument(
    "--reeval-budget-mult",
    type=float,
    default=3.0,
    help="Multiplier for learn-budget during top-k reevaluation",
)
parser.add_argument(
    "--visualize",
...
MORPH_WEIGHT_START = float(args.morph_weight_start)
MORPH_WEIGHT_END = float(args.morph_weight_end)
REEVAL_TOP_FRAC = min(1.0, max(0.0, float(args.reeval_top_frac)))
REEVAL_BUDGET_MULT = max(1.0, float(args.reeval_budget_mult))
...
class MinimalJointEvolution:
    def __init__(self) -> None:
        ...
        # evaluate() is called once for initial population plus once per EA step.
        self._eval_round = 0
        self._total_eval_rounds = BUDGET + 1

    def _current_morph_weight(self) -> float:
        if self._total_eval_rounds <= 1:
            return MORPH_WEIGHT_END
        alpha = min(1.0, max(0.0, self._eval_round / (self._total_eval_rounds - 1)))
        return MORPH_WEIGHT_START + alpha * (MORPH_WEIGHT_END - MORPH_WEIGHT_START)
    ...
    def evaluate(self, population: Population) -> Population:
        ...
        morph_weight = self._current_morph_weight()
        ...
        results_by_ind: dict[int, tuple[float, list[float]]] = {}
        ...
        if EVAL_WORKERS == 1:
            for ind, task in track(zip(to_eval, tasks), total=len(to_eval), description="Learning + Evaluating..."):
                ...
                results_by_ind[id(ind)] = (fit, best_vec)
        else:
                ...
                while pending:
                    ...
                    if now >= deadline:
                        ...
                        for fut in pending:
                            ...
                            results_by_ind[id(ind)] = (float("inf"), [])
                        break
        ...
        # Stage-2 reevaluation for top-performing morphologies with higher inner budget.
        if REEVAL_TOP_FRAC > 0.0 and to_eval:
            ranked = sorted(
                to_eval,
                key=lambda ind: results_by_ind.get(id(ind), (float("inf"), []))[0],
            )
            k = max(1, int(np.ceil(len(ranked) * REEVAL_TOP_FRAC)))
            topk = ranked[:k]
            reeval_budget = max(LEARN_BUDGET + 1, int(np.ceil(LEARN_BUDGET * REEVAL_BUDGET_MULT)))
            ...
            if EVAL_WORKERS == 1:
                for ind, task in zip(topk, reeval_tasks):
                    fit, best_vec = _evaluate_individual_process(task)
                    results_by_ind[id(ind)] = (fit, best_vec)
            else:
                ctx = mp.get_context("spawn")
            ...
            console.log(
                "[cyan]Stage-2 reevaluation:[/cyan] "
                f"top={k}/{len(to_eval)}, reeval_learn_budget={reeval_budget}",
            )

        for ind in to_eval:
            fit, best_vec = results_by_ind.get(id(ind), (float("inf"), []))
            ind.fitness = fit
...
def main() -> None:
    ...
    console.log(
        ...
        f"Dur={DURATION}s, Delay={EVAL_DELAY}s, MorphWeight=[{MORPH_WEIGHT_START:.3f}->{MORPH_WEIGHT_END:.3f}], "
        f"ReevalTopFrac={REEVAL_TOP_FRAC:.2f}, ReevalBudgetMult={REEVAL_BUDGET_MULT:.2f}",
    )
