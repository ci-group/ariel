"""
Morphology-only evolution for CPPN genomes using the cube pruning decoder.
Same morphological fitness as tree-based example.
"""
...
# Configuration
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Morphology-only CPPN evolution (cube decoder)")
parser.add_argument("--budget", type=int, default=50)
parser.add_argument("--pop", type=int, default=100)
parser.add_argument("--max-modules", type=int, default=15)
parser.add_argument("--visualize", action="store_true", help="Launch MuJoCo viewer for best individual")
...
class CPPNCubeEvolution:
    ...
    def decode_to_graph(self, genome: Genome):
        decoder = MorphologyDecoderCubePruning(cppn_genome=genome, max_modules=NUM_MODULES)
...
if __name__ == "__main__":
    console.log(f"Starting CPPN morphology-only evolution (cube decoder): pop={POP_SIZE}, gen={BUDGET}")
    evo = CPPNCubeEvolution()
    start = time.time()
    best = evo.evolve()
