...
POP_SIZE = 16  # 16 cores: use 32 (processes 16 at a time, then 16 more)
NUM_GENERATIONS = 5  # Increased: parallel speedup enables more generations

# Controller learning parameters
LEARNING_POP_SIZE = 5  # CMA-ES candidates per iteration
LEARNING_BUDGET = 8  # Total CMA-ES budget = 5 * 8 = 40 evals per morphology
SIMULATION_DURATION = 5  # Duration per controller evaluation
...
def evaluate_single_individual(ind: Individual) -> tuple[float, list | None]:
    """Worker function to evaluate a single individual in its own thread."""
    ...
    try:
        ...
        for _ in range(LEARNING_BUDGET):
            ...
            for candidate in candidates:
                ...
                # Use the new thread-safe runner (NO global callback!)
                thread_safe_runner(model, data, controller, SIMULATION_DURATION)
        ...
        # Catch errors gracefully so one bad morphology doesn't crash the thread pool
        return float("inf"), None

def evaluate(population: Population) -> Population:
    """Evaluate population by learning optimal neural network controllers using multithreading."""
    to_eval = [
        ind
    ...
    console.log(f"[cyan]Dispatching {len(to_eval)} evaluations to ThreadPool...[/cyan]")
    
    # Execute evaluations in parallel
    with ThreadPoolExecutor() as executor:
        # executor.map preserves the order of results
        results = list(executor.map(evaluate_single_individual, to_eval))
        
    # Assign results back to individuals
    for ind, (fitness, best_weights) in zip(to_eval, results):
        ind.fitness = fitness
...
def main() -> EA:
    ...
    if best_individual:
        ...
        try:
            ...
            # Use the passive viewer so we can run our custom control loop manually
            with viewer.launch_passive(model, data) as v:
                # Run the viewer until the user closes the window
                while v.is_running():
                    # 1. Apply neural network control
                    ...
                    # 4. Try to run at roughly real-time (optional, prevents running too fast)
