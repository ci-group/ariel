...
-----
    Martin:
    Vectorized implementation of CPG equation (3) in a synchronous fashion.
    The dynamics differ to that of the original file na_cpg.py since it is sequential:
    In this case we update the new state considering only the old one, not what is written live.
...
class NaCPG(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Adjacency mask: adj_mask[i, j] == 1 iff j is a neighbour of i
        adj_mask = torch.zeros(self.n, self.n)
        for i, neighbours in adjacency_dict.items():
            if neighbours:
                adj_mask[i, neighbours] = 1.0
    ...
    def forward(self, time: float | None = None) -> torch.Tensor:
        ...
        with torch.inference_mode():
            ...
            xy = self.xy
            x, y = xy[:, 0], xy[:, 1]
            x_old, y_old = self.xy_dot_old[:, 0], self.xy_dot_old[:, 1]

            # Coupling: sum_j R_ij @ xy_j over the neighbours of i, where R_ij is the rotation by the phase difference (phase_i - phase_j).
            phase_diff = self.phase[:, None] - self.phase[None, :]
            cos_d, sin_d = torch.cos(phase_diff), torch.sin(phase_diff)
            coupling_x = ((cos_d * x[None, :] - sin_d * y[None, :]) * self.adj_mask).sum(1)
            coupling_y = ((sin_d * x[None, :] + cos_d * y[None, :]) * self.adj_mask).sum(1)

            #K_i @ xy_i
            term_a = self.term_a(self.alpha, x**2 + y**2)
            term_b = self.term_b(self.zeta(self.ha, x_old), self.w)

            x_dot = term_a * x - term_b * y + coupling_x
            y_dot = term_b * x + term_a * y + coupling_y

            # Constraint function
            diff = 10
            x_dot = torch.clamp(x_dot, x_old - diff, x_old + diff)
            y_dot = torch.clamp(y_dot, y_old - diff, y_old + diff)

            # Advance every oscillator, then roll the state history over
            new_xy = torch.stack((x + x_dot * self.dt, y + y_dot * self.dt), dim=1)
            self.xy_dot_old.copy_(xy)
            self.xy.copy_(new_xy)

            # Save the angles (results)
            angles = self.amplitudes * new_xy[:, 1]
