...
def is_single_connected_tree(genome: Dict) -> bool:
    ...
    # `nodes` may have string keys (from to_dict()) or integer keys (internal
    # representation).  IDX_OF_CORE is always an integer, so normalise once.
    # Without this, `IDX_OF_CORE not in {"0": ...}` is always True because
    # Python never considers 0 == "0".
    if IDX_OF_CORE not in nodes and str(IDX_OF_CORE) not in nodes:
        return False

    # Build adjacency list with integer keys so IDX_OF_CORE lookups work
    # regardless of whether the incoming dict has string or integer keys.
