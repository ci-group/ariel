...
def is_single_connected_tree(genome: dict[str, Any]) -> bool:
    ...
    for e in edges:
        parent = _to_int_node_id(
            e["parent"]
        )
        child = _to_int_node_id(
            e["child"]
        )

        if (
            parent in adj_list
            and child in adj_list
        ):
...
def validate_genome_dict(
    ...
    if (
        str(IDX_OF_CORE) not in nodes
        and IDX_OF_CORE not in nodes
    ):
        msg = (
            f"Genome must contain core node "
            f"with index {IDX_OF_CORE}"
        )
    ...
    for k, v in list(nodes.items()):
        ...
        if (
            ...
            if not (
                ...
                msg = (
                    f"Node {nid} has brick length "
                    f"{length}, which must be between "
    ...
    for e in edges:
        ...
        if face not in ModuleFaces.__members__:
            msg = (
                f"Edge parent={parent} "
                f"child={child} has invalid "
                f"face '{face}'"
            )
            raise ValueError(msg)
        ...
        ptype = (
            nodes[str(parent)]["type"]
            if str(parent) in nodes
            else nodes[parent]["type"]
        )
        ...
        if face not in allowed:
            msg = (
                f"Face '{face}' not allowed "
                f"for parent type '{ptype}'"
            )
            raise ValueError(msg)
        ...
        if key in occupied:
            msg = (
                f"Parent {parent} already "
                f"has child at face '{face}'"
            )
            raise ValueError(msg)
    ...
    if not is_single_connected_tree(genome):
        msg = (
            "Genome does not form a single "
            "connected tree with core as root"
        )
        raise ValueError(msg)