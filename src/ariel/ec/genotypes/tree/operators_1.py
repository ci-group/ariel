...
def subtree_swap(a: TreeGenome, b: TreeGenome, a_node: int, b_node: int) -> None:
    ...
    def extract_subtree(genome: TreeGenome, root: int):
        g = genome.to_networkx()
        subnodes = {root, *list(nx.descendants(g, root))}
...
def mutate_subtree_replacement(genome: TreeGenome, max_modules: int = 10) -> None:
    ...
    if parent_id is not None and parent_face is not None:
        ...
        if IDX_OF_CORE in new_subtree.nodes:
            ...
            core_children = [(e["child"], e["face"]) for e in new_subtree.edges if e["parent"] == IDX_OF_CORE]
            ...
            for child_id, face in core_children:
                if child_id in new_nodes:  # Should always be true after reassignment
                    new_child_id = next(k for k, v in new_nodes.items() if v == new_subtree.nodes[child_id])
...
def mutate_hoist(genome: TreeGenome) -> None:
    ...
    nodes_with_children.update(e["parent"] for e in genome.edges)
...
def random_tree(max_modules: int) -> TreeGenome:
    ...
    while next_id <= max_modules:
        ...
        for pid, pdata in g.nodes.items():
            ...
            free.extend((pid, face) for face in allowed_faces if face not in used)
        if not free:
            break
...
def get_tree_depth(genome: TreeGenome) -> int:
    ...
    while stack:
        ...
        stack.extend((child, depth + 1) for child in g.successors(node))
...
def _remove_orphaned_nodes(genome: TreeGenome) -> None:
    """Remove nodes that are not reachable from IDX_OF_CORE via current edges.

    After structural edits (type changes, subtree removal, edge pruning) some
    nodes can become stranded — they still exist in ``genome.nodes`` but have
    no path from the core.  This pass does a BFS from the core and deletes any
    node not reached, then strips the edges that reference those deleted nodes.
    """
    if IDX_OF_CORE not in genome.nodes:
        return
    visited: set[int] = set()
    stack = [IDX_OF_CORE]
    while stack:
        ...
        if node in visited:
            continue
        visited.add(node)
        stack.extend(e["child"] for e in genome.edges if e["parent"] == node and e["child"] not in visited)
    orphans = [nid for nid in list(genome.nodes) if nid not in visited]
    for nid in orphans:
    ...
    if orphans:
        ...
        genome.edges = [
            ...
            if e["parent"] not in orphan_set and e["child"] not in orphan_set
        ]
...
def _prune_invalid_edges(genome: TreeGenome) -> None:
    ...
    Bug fix: ``valid_edges`` is re-filtered after ``remove_subtree`` calls
    because those calls can delete nodes that appeared valid during the scan,
    leaving ``valid_edges`` with stale references.  A final orphan-node sweep
    ensures genome connectivity is preserved.
    """
    ...
    # Re-filter: remove_subtree may have deleted nodes that were considered
    # valid during the scan above, leaving stale references in valid_edges.
    genome.edges = [
        e for e in valid_edges
        if e["parent"] in genome.nodes and e["child"] in genome.nodes
    ]

    # Final sweep: remove any nodes no longer reachable from the core.
    _remove_orphaned_nodes(genome)
