...
def remove_subtree(genome: TreeGenome, node_id: int) -> None:
    ...
    genome.edges = [
        ...
        if e["child"] not in descendants
        and e["parent"] not in descendants
    ]
...
def subtree_swap(
    ...
    def extract_subtree(
        ...
        internal_edges = [
            e
            for e in genome.edges
            if e["parent"] in subnodes
            and e["child"] in subnodes
        ]
    ...
    def reassign_ids(
        ...
        *existing_ids*. A mapping dict is also returned but only callers that
        ...
        new_nodes = {
            mapping[old]: copy.deepcopy(attr)
            for old, attr in nodes.items()
        }
    ...
    b_nodes_new, b_edges_new, _ = reassign_ids(
        b_nodes,
        b_edges,
        a_existing,
    )
    a_nodes_new, a_edges_new, _ = reassign_ids(
        a_nodes,
        a_edges,
        b_existing,
    )
...
def crossover_subtree(
    ...
    Two offspring are produced by exchanging randomly chosen subtrees from each
    parent. Unlike one-point crossover on linear genomes, this selects any node
    (including leaves) and swaps the entire subtree rooted at that node. Returns
    a pair ``(child1, child2)``. Parents are left unmodified.
    """
...
def mutate_replace_node(
    ...
    rotations = [
        r.name
        for r in ALLOWED_ROTATIONS[ModuleType[new_type]]
    ]

    new_rot = (
        random.choice(rotations)
        if rotations
        else genome.nodes[nid]["rotation"]
    )
    ...
    allowed = [
        f.name
        for f in ALLOWED_FACES[ModuleType[new_type]]
    ]

    for e in list(genome.edges):
        if (
            e["parent"] == nid
            and e["face"] not in allowed
        ):
            remove_subtree(
                genome,
                e["child"],
...
def mutate_subtree_replacement(
    genome: TreeGenome,
    max_modules: int = 10,
) -> None:
    ...
    if (
        ...
        and parent_face is not None
    ):
        # Create a small random subtree (1-3 nodes typically)
        subtree_size = random.randint(
            1,
            min(3, max_modules),
        )
        ...
        if IDX_OF_CORE in new_subtree.nodes:
            ...
            if core_children:
                ...
                branch_edges = [
                    copy.deepcopy(e)
                    for e in new_subtree.edges
                    if e["parent"] in branch_nodes
                    and e["child"] in branch_nodes
                ]
                ...
                new_nodes, new_edges, mapping = reassign_ids(
                    branch_node_dict,
                    branch_edges,
                    existing_ids,
                )
...
def mutate_hoist(
    ...
    candidates = [
        nid
        for nid in nodes_with_children
        if nid != IDX_OF_CORE
    ]
    ...
    genome.edges = [
        ...
        for e in genome.edges
        if not (
            e["parent"] == grandparent_id
            and e["face"] == parent_face
        )
    ]

    genome.edges.append({
        "parent": grandparent_id,
        "child": child_to_hoist,
        "face": parent_face,
    })
...
def random_tree(
    ...
    while next_id <= max_modules:
        ...
        rotations = [
            r.name
            for r in ALLOWED_ROTATIONS[ModuleType[mtype]]
        ]
...
def _prune_invalid_edges(
    ...
    for e in list(genome.edges):
        ...
        if (
            parent_id not in genome.nodes
            or child_id not in genome.nodes
        ):
            continue

        parent_type = ModuleType[
            genome.nodes[parent_id]["type"]
        ]

        allowed = [
            f.name
            for f in ALLOWED_FACES[parent_type]
        ]
    ...
    genome.edges = [
        ...
        if e["parent"] in genome.nodes
        and e["child"] in genome.nodes
    ]