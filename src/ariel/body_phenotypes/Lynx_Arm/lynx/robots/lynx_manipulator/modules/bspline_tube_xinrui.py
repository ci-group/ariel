...
class BSplineTube(Module):
    ...
    def __init__(
        ...
        end_point_theta: float = 0.0,  # rad, rotation in YZ plane from +Z
        ...
        control_points: Optional[list[Vector3]] = None,  # Control points for B-spline
        count_joint_volumes: bool = True,  # Whether to account for joint volumes in end constraints
        ...
        self.color = color if color is not None else [0.1, 0.1, 0.1, 1.0]  # Default grey
    
        # Create attachment points at start and end
        attachment_points = { 
            self.ATTACH: AttachmentPoint(
                offset=end_point_pos,
    ...
    def _create_cylinder_segments(self):
        ...
        for i in range(len(self.curve_points) - 1):
            ...
            # print(f"[bspline tube] segment direction {direction}")
    ...
    def _end_dir_from_theta_yz(self, theta: float) -> Vector3:
        # rotate +Z toward +Y in YZ plane
        # theta>0: clockwise (as you defined) -> +Y direction
        s, c = np.sin(theta), np.cos(theta)
        return Vector3([0.0, s, c]).normalize()

    def set_end_constraints(self, end_point_pos: Vector3, end_point_theta: float, dual_point_distance: float):
        """
        Re-parameterize the tube with 4 control points (clamped cubic) so that:
        - start point is (0,0,0) and start tangent is strictly +Z
        - end point is end_point_pos and end tangent follows end_point_theta
    ...
    def build(self, mjcf, entry_point, attachment_point_pos, attachment_point_quat):
        ...
        for i, segment in enumerate(self.cylinder_segments):
            ...
            main_body.add(
                ...
                # contype=1 << self.GROUP_TUBE,
                # conaffinity=(1 << self.GROUP_JOINT) | (1 << self.GROUP_ENV),
                contype=0,
                conaffinity=0,
                # density=0.0,
                mass=1e-6,
                ...
                pos=seg_pos,
                quat=seg_quat,
                rgba=self.color,  # orange: [1., 0.745, 0.298, 1]  blue grey: [0.68, 0.76, 0.82, 1.0]
                # density=800.0,
