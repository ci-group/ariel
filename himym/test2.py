...
WORLD_SIZE = [15, 15]  # Size of the flat world
...
def create_individual(num_joints):
    ...
    individual.tags = {
        "x": np.random.uniform(-WORLD_SIZE[0]+1, WORLD_SIZE[1]-1),
        "y": np.random.uniform(-WORLD_SIZE[0]+1, WORLD_SIZE[1]-1)
    }
    return individual
...
def distance_based_selection(population, dist_tree):
    """
    1. Create list of parents and sort them by fitness.
    2. Choose the fittest parent and find nearest neighbour.
    3. If the neighbour fitness is range 50% of original parent, we pair them
    4. Repeat the process till everyone is paired
    """
    sorted_inds = sorted(population, key=lambda ind: ind.fitness, reverse=True)
    selected = []
    used_indices = set()
    for i, ind in enumerate(sorted_inds):
        ...
        distances, indices = dist_tree.query([ind.tags["x"], ind.tags["y"]], k=len(sorted_inds))
        for dist, idx in zip(distances[1:], indices[1:]):  # Skip self (first result)
            if idx not in used_indices:
                neighbour = sorted_inds[idx]
                if neighbour.fitness >= 0.5 * ind.fitness:
...
def main():
    ...
    mujoco.set_mjcb_control(None)
    world = SimpleFlatWorld(floor_size=[WORLD_SIZE[0], WORLD_SIZE[1], 0.1])
    gecko_core = gecko()
    ...
    for gen in range(NUM_GENERATIONS):
        ...
        gen_kd_tree = KDTree([(ind.tags["x"], ind.tags["y"]) for ind in POPULATION])
        ...
        for i, individual in enumerate(POPULATION):
            ...
            print(f"  Individual {i+1}: fitness = {fitness:.4f}, x={individual.tags['x']:.2f}, y={individual.tags['y']:.2f}")
        ...
        if gen < NUM_GENERATIONS - 1:
            ...
            parents = distance_based_selection(POPULATION, gen_kd_tree)

            new_population = []
            for i in range(0, len(parents), 2):
                if i + 1 < len(parents):
                    print(f" Breeding parents {i} and {i+1} with fitness {parents[i].fitness:.4f}, {parents[i+1].fitness:.4f}")
                    print(f"  Parent genotypes: ({parents[i].tags['x']:.2f}, {parents[i].tags['y']:.2f}), ({parents[i+1].tags['x']:.2f},{parents[i+1].tags['y']:.2f})")
                    # CROSSOVER
